...
修改 profile 或设备路径后，应通过菜单重新加载设备。插件会先释放插件自有旧节点，再打开新节点。

配置加载会校验 profile 结构、输入节点格式和数值范围。非法配置不会替换当前运行配置；唤醒延迟、冷却时间和轴阈值越界时使用默认值。插件不会自动修改 `device_path`，设备节点需要手动维护。

## 输入设备边界

KOReader 的事件 hook 会看到多个输入来源。插件使用三层过滤：
...
2. 通过 FBInk 输入能力识别，只接受 `INPUT_JOYSTICK` 或 `INPUT_DPAD`，并排除鼠标、指点杆、触摸板、触摸屏、手写板、加速度计、键盘、系统按键和旋转事件。
3. 真正处理事件时，只接受当前 profile 的 `device_path` 在 KOReader 中打开的 fd，并要求事件自身的 `ev.fd` 相同。
...
切换 profile、唤醒重连或重新加载插件自有设备时，日志出现关闭旧节点再打开新节点属于正常流程。KOReader 已经打开的共享设备不会被插件重复关闭。日志中的 `idx` 是 KOReader 内部输入设备数组索引，不是 `/dev/input/eventN`。

插件复用 KOReader 原生事件循环，只做一次当前 fd 查表和整数比较，不创建额外线程或轮询器。蓝牙状态优先使用 Kindle 的 `liblipclua`，只有不可用时才回退到 `lipc-*` 命令。

KOReader 的“禁用按键重复”设置可能重置输入 hook；插件会在下一个 UI tick 恢复自己的 hook，并在设置生效时忽略 `KEY_REPEAT`。
...
- 只有成功读取过配置文件才允许回写；配置先写入 `.tmp` 并刷盘，再原子替换正式文件，按条件保留 `.old` 备份。
